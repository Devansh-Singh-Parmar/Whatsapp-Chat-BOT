  ...
  client.onMessage((message) => {
    ...
    switch (blocks[0]) {
        ...
        Money.findOne({ name: senderno }, function (err, response) {
          ...
          if (response === null || response == undefined) {
            ...
            client
              .reply(
                message.from,
                "Created New account for " + senderno + " with 1500 bucks cash",
                message.id
              )
              .then((result) => {
      ...
      case "give":
        ...
        Money.findOne({ name: senderno }, function (err, result) {
          if (result !== null) {
            ...
            client
              .reply(message.from, "Pls Make a bank account first", message.id)
              .then((result) => {
                console.log("Result: ", result);
              })
              .catch((erro) => {
      ...
      case "math":
        ...
        axios
          .get("http://api.mathjs.org/v4/?expr=" + encodeURIComponent(fullmsg))
          .then(function (response) {
            client
        ...
        Money.find()
          ...
          .exec(function (error, data) {
            ...
            for (var i = 0; i < data.length; i++) {
              richest += data[i].name + " : " + data[i].money.toFixed(2) + "\n";
            }
            client
      ...
      case "gareeb":
        Money.find()
          ...
          .exec(function (error, data) {
            ...
            for (var i = 0; i < data.length; i++) {
              gareeb += data[i].name + " : " + data[i].money.toFixed(2) + "\n";
            }
            client
